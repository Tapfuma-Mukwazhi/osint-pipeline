...
- [x] OSINT tooling wrapper (`crtsh_subdomains.py`) -- passive subdomain enumeration via certificate transparency logs, the same technique theHarvester's `crtsh` module uses. Built from scratch instead of wrapping the actual theHarvester binary, since the PyPI package for it is a stale non-functional placeholder and the real tool isn't designed for headless CI use -- see the module docstring for the full reasoning. Its first live run hit a real `502` from crt.sh (a free community service that's well known to be flaky under load) -- added retry-with-backoff rather than treating it as a one-off, since the same failure will recur.
